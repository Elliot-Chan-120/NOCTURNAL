...
The molecules generated by MutaGen can then be visualized in a Chemical Space Network graph where they are separated by similarity. You can see the chemical molecules show up, as well as hover your mouse over the chemical structures / nodes to see the information, such as its string format (e.g. O=C(CCl)NOCl), it's % rank by potency and its pIC50 value.
