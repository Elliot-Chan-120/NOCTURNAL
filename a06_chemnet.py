...
class ChemNet:
    ...
    def graph_data(self):
        ...
        top_keys = [key for key, tan_mcs in sorted_subset[:top_n]]
    ...
    def interactive_network(self, subsets, nodes):
        ...
        fig = go.Figure(data=[edge_trace, node_trace],
                        layout=go.Layout(
                            title=dict(
                                text=f"Chemical Space Network Graph: {self.model_name} {self.network_type} compounds - Top {self.top_percent}% similarity",
                                font=dict(size=25)),
                            images=node_images,
                            showlegend=False,
                            hovermode='closest',
